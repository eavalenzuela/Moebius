-- Moebius reference schema
-- Development-time only. Will be frozen into a migration before first deployment.
-- Sources: AGENT_CHECKIN_AND_CORE_DESIGN_SPEC, AGENT_AUTH_SPEC, FILE_TRANSFER_SPEC,
--          AGENT_UPDATE_SPEC, INSTALLER_PACKAGING_SPEC
...
CREATE TABLE roles (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id   UUID REFERENCES tenants(id),  -- NULL = system (predefined) role
    name        TEXT NOT NULL,
