...
| I8 | Release artifacts and agent updates verified via Ed25519 signature + SHA-256 | SECURITY | ☑ | ☑ | ☑ | Agent update executor verifies SHA-256 + Ed25519 sig **before** staging binary. Signing toolchain uses `crypto/ed25519` + `crypto/rand`. **Landed 2026-04-13:** server-side file sig verification in `files.go:CompleteUpload` + defense-in-depth check in `Download`; tenant filter on `agent_signingkeys.Get`. **Deferred to downstream packagers:** `keys/release.pub` real keypair, installer first-install verification. See I8 notes. |
...
- Code Reviewed:
  ...
  - **Agent-facing key endpoint (`server/api/agent_signingkeys.go:29-53`):** `GET /v1/agents/signing-keys/{key_id}` — returns public key PEM. Inside mTLS route group. Query now scopes `WHERE id = $1 AND tenant_id = $2` with the tenant resolved from mTLS context, so a cross-tenant key ID returns 404. **Landed 2026-04-13** — addresses the tenant-filter gap called out in the original review. Regression test: `TestSecurity_AgentSigningKeyCrossTenantReturns404`.
  ...
  - **File model (`shared/models/file.go:16-30`):** `File` struct has `Signature`, `SignatureKeyID`, and `SignatureVerified` fields. **Landed 2026-04-13:** `server/api/files.go:CompleteUpload` now verifies the Ed25519 signature against the registered signing key's PEM (matched by `signature_key_id` + `tenant_id`) over the SHA-256 digest produced during chunk assembly, mirroring the agent verify path. On failure: the assembled object is deleted from the backing store, `signature_verified` stays FALSE, `storage_path` is not set, and the response is `400 signature_invalid`. Defense-in-depth: `Download` refuses to serve any row that has a non-empty `signature` but `signature_verified=FALSE`. The PEM parser is shared with `signingkeys.Create` via `parseEd25519PublicKeyPEM`.
- Tests Created:
...
- Tests Added (2026-04-13):
  - `TestSecurity_FileSignature_ValidSignatureAccepted` — registers a real Ed25519 key, signs a file's SHA-256, walks the full upload flow, and asserts the DB row lands with `signature_verified=TRUE`.
  - `TestSecurity_FileSignature_InvalidSignatureRejected` — signs *different* content than is uploaded, expects `400 signature_invalid` from `CompleteUpload`, and asserts the row is left with `signature_verified=FALSE` / `storage_path=""`.
  - `TestSecurity_AgentSigningKeyCrossTenantReturns404` — inserts signing keys in two tenants, enrolls a tenant-1 agent, confirms same-tenant lookup is 200 and cross-tenant lookup is 404.
- Deferred to downstream packagers (upstream is not the primary release channel):
  - **`keys/release.pub` real keypair + CI registration.** The file is still the placeholder it was during review. This is intentional upstream: the agent verify path fails closed when the key is absent, which is correct upstream behaviour. Generating a real Ed25519 keypair, checking in the public half, enrolling it on the server, and operating signing in CI are the responsibility of whoever operates the downstream release pipeline.
  - **Installer first-install verification.** The initial install flow (enrollment script downloads installer) trusts the TLS channel and the enrollment token, not a signature. Moving to a signed-manifest or per-package-manager signing story depends on the downstream distribution channel (MSI / rpm / deb / S3) and is not a decision upstream can make unilaterally.
- Accepted Risk:
  - **Server-side signature verification does not rehash the file from disk.** `CompleteUpload` verifies against the SHA-256 digest computed during chunk assembly (the same digest that was matched against `expected_sha256`), not by re-reading the assembled blob. This is correct given the assembly hasher is authoritative for the bytes that were written, but it does mean a corruption introduced *after* `CompleteUpload` on the backing store would not be caught until `Download` (which does not currently rehash either). See "No whole-store re-verification background job" below.
  - **No whole-store re-verification background job.** Once a file's `signature_verified` flag flips to TRUE, there is no scheduled re-verification. On-disk corruption or backend tampering after the fact would not be caught unless the download path is extended to re-hash + re-verify. Tracked as a defense-in-depth hardening item rather than a deliverable of this pass.
- Notes / follow-up items:
...
- [x] Revoked keys rejected immediately (no cache TTL > a few seconds, or cache invalidation on revoke). **Evidence:** `server/auth/apikey.go:87-98` queries `api_keys WHERE key_hash = $1` on every request — no in-memory cache. Revocation is hard delete (`DELETE FROM api_keys`), so the next request returns `pgx.ErrNoRows` → 401. Integration test: `TestSecurity_RevokedAPIKeyRejectedImmediately`.
...
- [x] Token scope (group/tag/site) is copied to the device at enrollment and cannot be escaped. **Evidence:** `server/api/enroll.go:applyScope` writes `device_groups`/`device_tags`/`device_sites` rows from the token's stored scope. Agent has no endpoint to modify its own group/tag/site memberships post-enrollment (grep across `server/api/`: only operator-side store handlers write these tables, all RBAC-protected). **Remediated during audit:** (1) enrollment is now wrapped in a single transaction (device + scope + cert), so partial scope failure rolls the whole enrollment back instead of leaving an unscoped device — was previously logged as "non-fatal" at `enroll.go:88`; (2) `ValidateScopeTenant` (new in `server/auth/scope.go`) is called at token creation to reject scope IDs that belong to a different tenant — without this, FK constraints alone allowed cross-tenant pollution of `device_groups` since the membership table has no `tenant_id` column; (3) `applyScope` defends in depth at enrollment time by inserting only when the parent group/tag/site exists in the same tenant. Integration tests: `TestSecurity_EnrollmentTokenScopeCopiedToDevice`, `TestSecurity_EnrollmentRejectsCrossTenantScope`, `TestSecurity_EnrollmentRollsBackOnBadScope`.

**OIDC/SSO**

**Status note:** OIDC middleware (`server/auth/oidc.go`) is implemented but **not currently wired into the router** — `NewOIDCMiddleware` is never called from `server/api/router.go` or `server/cmd/api/main.go`, and `OIDCConfig` env vars (`OIDC_ISSUER_URL`, `OIDC_CLIENT_ID`, `OIDC_CLIENT_SECRET`) are read in `server/config/config.go` but never consumed. The feature is dormant — no SSO user can authenticate today. The audit below evaluates the code as it stands so that wiring it up is a one-line change rather than a re-audit.

- [x] ID token signature verified against JWKS. **Evidence:** `oidc.go:42` builds the verifier via `provider.Verifier(&oidc.Config{ClientID: cfg.ClientID})`. The `coreos/go-oidc/v3` library's `Verify()` (called at `oidc.go:65`) fetches the JWKS from the provider's discovery document (`/.well-known/openid-configuration` → `jwks_uri`) and validates the JWT signature against the published keys. No custom signature handling that could bypass verification.
- [x] `aud` and `iss` claims validated. **Evidence:** `oidc.Config{ClientID: cfg.ClientID}` binds the verifier to the configured audience; `provider.Verifier` is bound to the issuer URL passed to `NewProvider`. The library rejects tokens whose `iss` does not match the discovered issuer or whose `aud` does not contain `ClientID`.
- [x] Token expiry enforced. **Evidence:** `go-oidc` `Verify()` validates `exp` (and `nbf` when present) on every call. No caching of verified tokens — each request re-verifies.
- [x] User-to-tenant mapping cannot be spoofed via claims the user controls. **Evidence:** `oidc.go:97-103` extracts `tenantID` and `roleID` from the `users` table via `JOIN roles` keyed only on `sso_subject` from the verified `sub` claim. **No claim from the token is used as input to the tenant assignment** — `email` is read but discarded; `iss`, `aud`, custom claims are never consulted for authorization. Tenant safety therefore reduces to the integrity of the `sso_subject → users.tenant_id` mapping.

  **Remediated during audit:** `users.sso_subject` previously had no UNIQUE constraint, so two users in different tenants could carry the same subject and the lookup would return one arbitrarily. Migration `005_users_sso_subject_unique.up.sql` adds a partial unique index `users_sso_subject_unique ON users(sso_subject) WHERE sso_subject IS NOT NULL` — duplicates are now rejected at insert time, while NULL (most users today) is unaffected. Integration test: `TestSecurity_OIDCSubjectUnique`.

  **Remaining gaps (filed under accepted risk pending product decision):**
  - **No SSO provisioning path.** `store.CreateUser` does not insert `sso_subject`, and no API endpoint sets it. Today the column is always NULL after user creation, so the OIDC lookup always returns `unknown_sso_user`. Wiring up OIDC will require either an admin endpoint that links a user to an SSO subject or a JIT-provisioning flow. The unique index above is a prerequisite either way.
  - **No provider isolation.** `sso_subject` is a single column with no link to which OIDC issuer minted the subject. Multi-IdP deployments where two providers happen to issue overlapping `sub` values would collide on the unique index — safe (insert rejected), but operationally awkward. Future work: store `(issuer, subject)` and key the lookup on both. Single-IdP deployments are unaffected.
  - **OIDC middleware not wired.** Adding `r.Use(oidcAuth.Handler)` after the API key middleware will activate it; the middleware is already a no-op for `sk_`-prefixed bearer tokens so the two auth methods can coexist.

### 2.2 Authorization (RBAC + Scope)

**Predefined role matrix**
- [x] For each of Super Admin, Tenant Admin, Operator, Technician, Viewer: test a representative set of endpoints (read, write, admin) and confirm the permission matrix in `server/rbac/` matches `SECURITY.md`. **Evidence:** Per-role coverage in `tests/integration/rbac_test.go`: Super Admin = `TestRBAC_AdminBypassesAll` (15 endpoints, `is_admin=true`), Tenant Admin = `TestRBAC_TenantAdminHasFullTenantAccess` (same 15 + token mint, permission-based without `is_admin`), Operator = `TestRBAC_OperatorCanManageMostButNotUsersOrRoles`, Technician = `TestRBAC_TechnicianCanCreateJobsButNotManageUsers`, Viewer = `TestRBAC_ViewerCanReadButNotWrite`. Permission sets sourced from canonical lists in `server/rbac/permissions.go` so any drift between SECURITY.md and the code shows up immediately. Custom-role coverage: `TestRBAC_CustomRoleWithPartialPerms`.
...
- [x] Can a Tenant Admin create a role with cross-tenant permissions? (must not) **Evidence:** Code fix in `server/api/roles.go:Create` and `Update` — non-admin callers must pass `auth.PermissionsSubset(callerPerms, req.Permissions)` (admin bypass intact). Without the clamp, anyone with `roles:write` could mint a role granting `tenant:write`/`users:write` and escalate via key/user assignment. Tests: `TestSecurity_PrivilegeEscalation_NonAdminCannotCreateRoleWithEscalatedPerms` (POST) and `TestSecurity_PrivilegeEscalation_NonAdminCannotUpdateRoleWithEscalatedPerms` (PATCH widening). The "cross-tenant permissions" phrasing in the original item is interpreted as "permissions outside the caller's own grant" — the role row itself is always tenant-scoped via `auth.TenantIDFromContext`.
- [x] Can a user assign themselves a higher role? (must not) **Evidence:** Code fix in `server/api/users.go:Update` — non-admin callers cannot edit their own `user_id` at all (`self_role_change` 403), and any cross-user role assignment must satisfy `PermissionsSubset(callerPerms, targetRole.Permissions)`. Test: `TestSecurity_PrivilegeEscalation_UserCannotSelfAssignHigherRole` exercises both the self-target reject and the cross-user clamp via a custom `users:write`-only role.
...
- [x] Can a non-admin revoke an admin's key or device? Verify consistent behavior. **Evidence:** Code fix in `server/api/apikeys.go:Delete` — handler now loads the target key via new `store.GetAPIKey` and rejects `is_admin=true` deletes from non-admin callers (`admin_key_protected` 403). Test: `TestSecurity_PrivilegeEscalation_NonAdminCannotDeleteAdminKey` confirms (1) admin-key delete is blocked, (2) admin key still works after the rejected delete, (3) non-admin keys remain deletable so the guard is targeted, not blanket. Device revocation (`devices.go:Revoke`) is already gated by scope enforcement (`auth.ResolveScope` + `DeviceInScope`), so a non-admin can only revoke devices in their assigned scope; covered by `TestSecurity_ScopedKeyCannotAccessOutOfScopeDevice`.
...
- [x] Max body size enforced on all endpoints (DoS protection). **Evidence:** New `MaxBytes(n int64)` middleware in `server/api/middleware.go` wraps `r.Body` with `http.MaxBytesReader`. Applied at the chi root via `r.Use(MaxBytes(MaxBodyBytesGlobal))` (8 MB) so every endpoint — including unauthenticated `/v1/agents/enroll`, `/health`, `/metrics`, and the entire `/v1` and `/v1/agents` trees — inherits a hard upper bound. Tighter per-route caps overlay where the expected body is larger than ordinary JSON: chunk upload (`MaxBodyBytesFileChunk` = 6 MB), agent check-in (`MaxBodyBytesAgentInventory` = 4 MB), agent log ingest (`MaxBodyBytesAgentLogs` = 4 MB), agent job result (`MaxBodyBytesAgentInventory`). Tests: `TestSecurity_BodySize_JSONCRUDRejectsOversizedBody` (12 MB JSON to `PATCH /v1/devices/...` returns 400/413), `TestSecurity_BodySize_GlobalCapBlocksHugeBody` (16 MB to unauthenticated enroll endpoint rejected). Tightening individual `/v1` JSON CRUD endpoints below the 8 MB global cap is left as a future hardening (chi forbids `r.Use` after routes are registered, so it would require restructuring all 80+ child routes into a sub-Group; the global cap already provides the DoS protection the requirement asks for).
- [x] Check-in payload: inventory size limits, delta size limits. **Evidence:** `POST /v1/agents/checkin` is wrapped with `r.With(MaxBytes(MaxBodyBytesAgentInventory))` (4 MB) in `server/api/router.go`. This bounds the JSON-decoded `protocol.CheckinRequest` (heartbeat + delta inventory + telemetry) before `json.NewDecoder` runs, so an agent that ships a malformed huge inventory cannot exhaust server memory. Job result POSTs use the same cap to bound captured stdout/stderr.
- [x] Chunked file upload: per-chunk and total-size limits enforced; resumable upload cannot exceed declared size. **Evidence:** Two-layer per-chunk cap: (1) router applies `MaxBytes(MaxBodyBytesFileChunk)` (6 MB) on `PUT /v1/files/uploads/{upload_id}/chunks/{chunk_index}`, (2) the handler also wraps `r.Body = http.MaxBytesReader(w, r.Body, defaultChunkSize+1)` so an oversized chunk surfaces as a clean 400 instead of the previous `io.LimitReader` truncation that produced a confusing checksum mismatch (`server/api/files.go`). Total-size: `InitiateUpload` computes `totalChunks = ceil(SizeBytes / chunkSize)` from the declared size, the chunk handler rejects `chunk_index >= totalChunks` (line 192), and re-uploading the same index is idempotent via `array_append(array_remove(...))`, so the resumable upload cannot exceed the declared size. Test: `TestSecurity_BodySize_ChunkUploadAcceptsCappedSize` confirms a 1 MB chunk passes the cap (would 413 if the global JSON tight-cap leaked into the chunk route).
...
- [x] CSR inputs validated: reject CSRs with unexpected SAN, CN, or key usage. **Evidence:** I7 + audit — CSR signature validated (`CheckSignature`). CSR Subject/SANs are **ignored** — server controls identity by using `agentID` for CN and DNS SAN (line 86-90). This is secure-by-design: the agent cannot choose its own identity via CSR manipulation. Public-key type/curve is now enforced (see next item).
- [x] ECDSA P-256 enforced for agent keys; RSA / other curves rejected if not intended. **Evidence:** `pki.SignCSR` now calls `validateAgentPublicKey` (`server/pki/ca.go`) which type-asserts `*ecdsa.PublicKey` and confirms `Curve == elliptic.P256()`. RSA (any size), Ed25519, and non-P256 ECDSA curves (P-224/P-384/P-521) all return a typed error before `CreateCertificate` runs. The error path bubbles up through `EnrollHandler` as a clean `400 invalid CSR`. Tests: unit-level `TestSignCSR_RejectsNonP256Keys` (rsa2048, ecdsa_p384, ed25519 sub-cases) confirms each rejected key type, and integration tests `TestSecurity_Enrollment_RejectsRSACSR` + `TestSecurity_Enrollment_RejectsP384CSR` exercise the full HTTP path through `/v1/agents/enroll`, asserting both the 400 status and that no `devices` row is left behind. Locking the entire fleet to a single curve also matches what `GenerateCA` itself produces, so the CA and the certs it issues stay homogeneous.
...
- [x] Currently rotation guidance is scattered across multiple docs (release signing, CA, per-device CA, API keys). Consolidate into a single `docs/KEY_ROTATION.md` as a deliverable of this validation pass. **Evidence:** New `docs/KEY_ROTATION.md` is the canonical reference. The previous mentions in `SECURITY.md`, `AGENT_AUTH_SPEC.md`, and `LOCAL_UI_CLI_SPEC.md` remain in place (they describe the lifecycle of the key in question), but operators are now directed to one document for the actual procedures.
- [x] Document must cover: intermediate CA rotation, release signing key rotation, per-device local-UI CA regeneration, API key rotation cadence, DB password rotation, and rollback procedure for each. **Evidence:** `docs/KEY_ROTATION.md` has seven sections covering Root CA (§1), Intermediate CA (§2), Agent client certs (§3), Per-device Local UI CA (§4), Release signing key (§5), API keys (§6), and Database password (§7). Each section includes a Rollback subsection.
- [x] For each key, specify: storage location, rotation trigger (schedule vs. compromise), procedure, blast radius during rotation, and verification step. **Evidence:** Every section of `docs/KEY_ROTATION.md` follows the same template — Storage, Trigger (planned), Trigger (compromise), Procedure, Blast radius, Verification, Rollback. The intro spells out the template, and the Inventory Summary table at the bottom links every key to its section for incident-response navigation.
...
- [x] DB connection requires TLS in production config (`sslmode=require`). **Evidence:** New `Database TLS` section in `docs/Deployment_Instructions.md` documents `disable`/`require`/`verify-ca`/`verify-full` and explicitly requires `verify-full` (or `require` minimum) in production. Helm `values.yaml` `secrets.databaseUrl` carries an inline comment requiring `sslmode=require`/`verify-full`, with example connection strings. `deploy/docker-compose.yml` makes `DATABASE_URL` overridable via env (`${DATABASE_URL:-...}`) so operators can flip to `sslmode=require` without editing the compose file, and a header comment explains why the bundled-Postgres default is `sslmode=disable` (private docker network, evaluation only) and how to switch to TLS for production. Verification command (`SHOW ssl;` / `pg_stat_ssl`) included in the docs section. **Note:** the Helm secrets template was already correct in the deployment doc example (line 314); the gap was that nothing in the chart values or compose file made the production expectation explicit.
...
- [x] Test: agent reports CDM hold on check-in, server respects it. **Evidence:** `tests/integration/cdm_test.go:TestCDM_JobsHeldWithoutSession` — agent posts check-in with `CDMEnabled=true`, `CDMSessionActive=false`; server returns zero dispatched jobs and flips the queued job row to `JobStatusCDMHold`. Also `TestCDM_RevokeSessionHoldsNewJobs` covers the mid-session revoke path.
...
- [x] CDM session expiry: in-flight job completes, no new jobs start. **Evidence:** `tests/integration/cdm_test.go:TestCDM_SessionExpiryAllowsCompletionBlocksNewWork` — dispatches job A while session is active, acknowledges it, lets the session expire (agent flips `CDMSessionActive` to false), submits the job A result (must succeed — no CDM gate on `POST /v1/agents/jobs/{id}/result`), then re-checks in. Asserts (1) job A reaches `completed`, (2) the follow-up checkin returns zero new dispatches, and (3) the queued job B is moved to `JobStatusCDMHold`. This covers the full "in-flight finishes, new work blocked" loop.
...
- [x] Per-tenant resource limits: max devices, max jobs in queue, max file size, max API keys. **Evidence:** `server/quota/` — `Resolver` enforces count/size ceilings via `CheckDevices`, `CheckQueuedJobs`, `CheckAPIKeys`, `CheckFileSize`. Global defaults from `QUOTA_MAX_*` env vars (defaults: 10k devices, 10k queued jobs, 100 API keys, 1 GB files); per-tenant overrides via `TenantConfig.Quotas` JSONB; -1 = unlimited. Enforcement sites: `api/enroll.go` (after token consume), `api/apikeys.go` Create, `api/jobs.go` Create (fan-out atomic — rejected batch lands zero jobs) and Retry, `api/files.go` InitiateUpload (pre-insert). Rejections return 409 `quota_exceeded`. Queued-job count excludes terminal states so historical work does not pin the cap. Unit tests: `server/quota/quota_test.go` (overlay math, error format, nil-safe no-op). Integration tests: `tests/integration/quota_test.go` — `TestQuota_APIKeys_Exceeded`, `TestQuota_Devices_Exceeded`, `TestQuota_QueuedJobs_ExceededOnFanOut`, `TestQuota_QueuedJobs_CountsOnlyActive`, `TestQuota_FileSize_Exceeded`, plus under-cap allow-path cases.
...
- [x] `go list -m all` + `govulncheck` — report known-vulnerable dependencies. **Evidence:** Original scan on go1.25.0 reported **13 called stdlib vulnerabilities** (`crypto/tls` ×3, `crypto/x509` ×4, `net/url` ×3, `encoding/asn1`, `encoding/pem`, `os`). **Remediated:** `go.mod` now declares `toolchain go1.25.9`. Re-scan with `govulncheck ./...` reports **0 called vulnerabilities**. Two non-called pgx import findings (GO-2026-4771, GO-2026-4772 / CVE-2026-33815, CVE-2026-33816) remain — both have `Fixed in: N/A`, neither is reachable from any call path in our binaries. Tracked as accepted risk pending an upstream pgx release.
...
6. **Follow-on (post-validation):** `docs/THREAT_MODEL.md` — STRIDE per component. Built *after* this validation pass so the threat model reflects tested reality, not aspiration. **Landed 2026-04-13.**
